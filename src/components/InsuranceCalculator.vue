<template>
    ...
    <div class="first-row">
      <div class="form-row mb-4" v-for="(insurance, index) in values" :key="index">
        <div class="radio-wrapper">
          ...
          />
          <label class="form-check-label" :for="insurance.code">
            {{
              insurance.name
            }}
          </label>
    ...
    <div class="form-row mb-4">
      ...
      <div class="d-flex">
        <div
          ...
          v-for="(additional, index) in values[insurancePicked]
        ...
        >
          <input
            ...
            :value="index"
          />
          <label class="form-check-label" :for="additional.code">
            {{
              additional.name
            }}
          </label>
  ...
  <div
    class="result"
    v-if="
      (!attemptSubmit && compareDates) ||
      this.values[insurancePicked].code == 'year'
    "
  >
    <FormResult
      ...
      :insurancePackage="
        values[insurancePicked].packages[insurancePackage].name
      "
...
export default {
  ...
  computed: {
    calculateFinalCost: function () {
      ...
      return this.finalCost.toLocaleString("sk-SK", {
        style: "currency",
        currency: "EUR",
    ...
    pickedAdditionalInsurances: function () {
      ...
      for (const index of this.additionalInsurances) {
        result.push(
          this.values[this.insurancePicked].additionalInsurances[index].name
        );
      }
      return result;
    },
  ...
  methods: {
    ...
    getInsuranceDiffDays() {
      ...
      const diffDays = Math.ceil(
        Math.abs(dateB - dateA) / (1000 * 60 * 60 * 24)
      );
      return diffDays + 1;
    },
  },
